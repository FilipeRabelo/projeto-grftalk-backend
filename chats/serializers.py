...
class ChatSerializer(serializers.ModelSerializer):
  ...
  unseen_count = serializers.SerializerMethodField()   # msg nao vista
  ...
  def get_unseen_count(self, chat):
    unseen_count = ChatMessage.objects.filter(
      ...
      deleted_at__isnull = True       # significa q tem msg q nao foram vistas
    ).exclude(
  ...
  def get_last_message(self, chat):     # obter ultima msg recebida ou enviada
...
class ChatMessagesSerializer(serializers.ModelSerializer):
  ...
  def get_from_user(self, message):
    return UserSerializer(message.from_user).data   # para saber qual o user enviou a msg
  
  def get_attachments(self, message):

    if message.attachment_code == 'FILE':
      file_attachment = FileAttachment.objects.filter(
        id=message.attachment__id
      ).first()
