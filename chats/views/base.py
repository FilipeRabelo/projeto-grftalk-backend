...
class BaseView(APIView):
  ...
  # Get User by any field - Obter usuário por qualquer campo
  def get_user(self, raise_exception=True, **kwargs) -> User | None:   # **kwargs - argumentos nomeados
  ...
  # Verificando se o chat já existe para user_id e to_user   -   to_user_id
  def has_existing_chat(self, user_id, to_user) -> Chat | None:

    chat = Chat.objects.filter(
      (Q(from_user=user_id) & Q(to_user=to_user)) | (Q(from_user=to_user) & Q(to_user=user_id)), 
  ...
  # Checking if chat belong to user or not
  # Verificando se o chat pertence ao usuário ou não
